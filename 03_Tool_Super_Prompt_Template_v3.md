...
For structured prompts, use simple section tags like:
...
These tags label sections of a single prompt. They are not metadata or thread classifiers.

If you want a lighter syntax, you can instead use delimiters:
...
> **Note:** Prompt Lab metadata tags (e.g., classification labels) are our own convention inspired by XML structure. They are not part of Jeff Su's published frameworks.
...
> **Note:** For reasoning-capable models, start zero-shot. Only add exemplars if the model keeps getting the task wrong.
...
#### PROMPT OPTIMIZER (optional meta-prompt, run before Template A)

```
You are an expert prompt engineer specializing in GPT-5 (or the target
model). Your task is to take my draft prompt and transform it into a
well-crafted, effective prompt that follows best practices for this model.

- Clarify the Task, Context, Exemplars, Persona, Format, and Tone.
- Tighten the wording, remove redundancy, and highlight any missing
  information.
- Output only the optimized prompt in a fenced code block.
...
### 6. BLUEPRINT FIRST, THEN WRITE

```
Before writing the full output, outline the structure you will use and wait
for my approval.

- First, propose a clear outline or step-by-step plan for how you will
  structure the result.
- Use short section headings and one-sentence descriptions.
- Stop and wait for my confirmation or edits.
- Only after I confirm the outline, write the full output following that
  structure exactly.
```
